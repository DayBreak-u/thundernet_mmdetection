model = dict(
    ...
    backbone=dict(
        ...
        norm_eval=False,
        out_indices=(2, 3)),
    neck=dict(type='CEM', in_channels=[352, 1024], out_channels=216),
    rpn_head=dict(
        type='LightRPNHead',
        in_channels=216,
        feat_channels=512,
    ...
    roi_head=dict(
        ...
        bbox_roi_extractor=dict(
            ...
            roi_layer=dict(
                type='PSRoIAlign', output_size=(6, 6), sampling_ratio=2),
            out_channels=6,
            featmap_strides=[16]),
        bbox_head=dict(
            type='ConvFCBBoxHead',
...
train_pipeline = [
    ...
    dict(
        ...
        hue_delta=18),
    dict(type='Expand', mean=[0, 0, 0], to_rgb=False, ratio_range=(1, 4)),
    dict(
...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        ann_file='/data-tmp/coco2017/annotations/instances_train2017.json',
        img_prefix='/data-tmp/coco2017/train2017/',
        pipeline=[
            dict(type='LoadImageFromFile', to_float32=True),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(
    ...
    val=dict(
        type='CocoDataset',
        ann_file='/data-tmp/coco2017/annotations/instances_val2017.json',
        img_prefix='/data-tmp/coco2017/val2017/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
